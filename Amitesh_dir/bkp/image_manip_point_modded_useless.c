...
Image pointilism( const Image in , unsigned int seed ) {
   ...
   int random_radius[num_pixels];// Random radius for each pixel
   ...
   for(int i=0; i<num_pixels; i++){
       ...
       for(int j = 0; j<random_radius[i]; j++){
           ...
           idx_top = (rand_pixels[i]-(cols*(j+1)) < 0) ? -1 : rand_pixels[i] - (cols*(j+1));
           idx_bot = (rand_pixels[i]+(cols*(j+1)) > rows*cols) ? -1 : rand_pixels[i] + (cols*(j+1));
           idx_left = (overflow) ? -1 : rand_pixels[i] - (j+1);
		   idx_right = ((rand_pixels[i]+(j+1)) % (cols) == 0) ? -1 : rand_pixels[i] + (j+1);
       }
   }
   //-------------------------------------------------------------------------------------------------//
   ...
   //----------- Applying the circular filters ---------------//
	for(int i = 0; i<num_pixels; i++){
		...
		for(int j = 0; j<radius; j++){
			pointilism.data[indices_top[i][j]] = pix;
			pointilism.data[indices_bot[i][j]] = pix;
			pointilism.data[indices_left[i][j]] = pix;
			pointilism.data[indices_right[i][j]] = pix;
			for(int k = 0; k<radius; k++){
				pointilism.data[top_right[i][j][k]] =  (top_right[i][j][k] ==-1)? pointilism.data[top_right[i][j][k]] : pix;
				pointilism.data[top_left[i][j][k]]  =  (top_left[i][j][k]  ==-1)? pointilism.data[top_left[i][j][k]]  : pix;
				pointilism.data[bot_right[i][j][k]] =  (bot_right[i][j][k] ==-1)? pointilism.data[bot_right[i][j][k]] : pix;
				pointilism.data[bot_left[i][j][k]] =   (bot_left[i][j][k]  ==-1)? pointilism.data[bot_left[i][j][k]] :  pix;
			}
		}
	}
   //-------------------------------------------------------------------------------------------------//
