...
int write_ppm( FILE *fp , const Image im ) {
    ...
    int numWritten = fwrite(im.data,sizeof(Pixel),size,fp);
    return numWritten;
   }
...
Image make_image_zero_padd (int rows, int cols, int N){
	// Pad N/2 zeros on each side of the image
	int zero_padded_rows = rows + (N-1); // (N/2 on each side)
	int zero_padded_cols = cols + (N-1); // (N/2 on each side)
	Image im = {NULL,zero_padded_rows,zero_padded_cols};
	im.data = calloc(zero_padded_rows*zero_padded_cols,sizeof(Pixel));
	if(!(im.data)){
		fprintf(stderr,"Unable to allocate memory");
	}
	return im;
